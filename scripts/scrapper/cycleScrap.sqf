...
_VRCycleParts = [["PartGeneric","1"],["PartWheel","1"],["PartEngine","1"],["PartFueltank","1"]];
...
_randLow = floor(random 2);
_randMid = floor(random 3);

_VRCycleParts select 0 set [1,_randMid];
_VRCycleParts select 1 set [1,_randLow];
_VRCycleParts select 2 set [1,_randFlip];
_VRCycleParts select 3 set [1,_randFlip];
...
_spawnCrate addMagazineCargoGlobal [(_VRCycleParts select 0) select 0, (_VRCycleParts select 0) select 1];
